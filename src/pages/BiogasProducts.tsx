...
import { BIOGAS_PRODUCTS } from '../lib/mock-data';
import ProductCard from '../components/ProductCard';
import { Button } from '../components/ui/button';
import { Input } from '../components/ui/input';
import { Search, Droplets, SlidersHorizontal } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const BiogasProducts = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const urlCategory = searchParams.get('category');
  const [activeCategory, setActiveCategory] = useState(
    urlCategory && ['Biogas Systems', 'Accessories'].includes(urlCategory)
      ? urlCategory
      : 'All'
  );
  const [searchQuery, setSearchQuery] = useState('');

  const categories = ['All', 'Biogas Systems', 'Accessories'];
  ...
  const filteredProducts = useMemo(() => {
    let result = activeCategory === 'All'
      ? BIOGAS_PRODUCTS
      : BIOGAS_PRODUCTS.filter(p => p.category === activeCategory);
    if (searchQuery.trim()) {
        ...
        <div className="container mx-auto px-4 py-16 md:py-24 relative z-10">
          <div className="max-w-2xl space-y-6">
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight leading-tight">
              Land & Trust Biogas Solutions
            </h1>
            <p className="text-xl text-gray-400">
              Turn waste into a valuable resource. Explore our range of residential and commercial biogas systems.
            </p>
            <div className="flex gap-4 pt-4">
              <Button size="lg" className="h-12 px-8" onClick={() => handleCategoryChange('All')}>Explore Systems</Button>
